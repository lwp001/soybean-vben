...
interface RouteState {
  ...
  /** 搜索的菜单 */
  searchMenus: AuthRoute.RouteList;
  /** 缓存的路由名称 */
  cacheRoutes: string[];
}
...
export const useRouteStore = defineStore('route-store', {
  ...
  actions: {
    ...
    isValidConstantRoute(name: string) {
      const constantRouteNames = getConstantRouteNames(constantRoutes);
      return constantRouteNames.includes(name) && name !== 'not-found';
    },
    /**
     * 处理权限路由
     * @param routes - 权限路由
     */
    handleAuthRoute(routes: AuthRoute.RouteList) {
    ...
    handleUpdateRootRedirect(routeKey: string) {
      ...
      const rootVueRoute = { ...ROOT_ROUTE, redirect: routeKey as string } as RouteRecordRaw;
      const rootRouteName: string = 'root';
    ...
    /** 初始化权限路由 */
    async initAuthRoute() {
      console.log('路由模式: ', this.authRouteMode);
    ...
    async reCacheRoute(name: string) {
      ...
      if (isCached) {
        this.addCacheRoute(name as string);
      }
    }
  }
});
